...
cat file_list.txt| grep -v Input | grep combined| awk '{ 
print \
"echo \x22#PBS -m abe" \
"\n#PBS -M "ENVIRON["email"] \
"\n#PBS -V" \
...
"\n\nmkdir "$5"-"$1"-"$2"_tmp" \
"\n\nmacs2 callpeak -t ../homer/"$5"-"$1"-"$2"_30M.bed -c ../homer/"$5"-"$1"-Input_30M.bed -n "$5"-"$1"-"$2" -f BED -g hs --keep-dup all --nomodel -p 1e-2 --broad --nomodel --extsize 300  -B --SPMR --tempdir "$5"-"$1"-"$2"_tmp" \
"\n\nchipReads=\x5C\x24(wc -l ../homer/"$5"-"$1"-"$2"_30M.bed | awk \x27\x7Bprintf \x5C\x22%f\x5C\x22,\x5C\x24"1"\x2F"1000000"\x7D\x27)" \
"\n\ninputReads=\x5C\x24(wc -l ../homer/"$5"-"$1"-Input_30M.bed| awk \x27\x7Bprintf \x5C\x22%f\x5C\x22,\x5C\x24"1"\x2F"1000000"\x7D\x27)" \
